...
AddonsManager.prototype = {
  ...
  close : function AddonsManager_close(aIgnoreFailures) {
    ...
    try {
      if (aomTabs[0].controller.tabs.length > 1) {
        let tabBrowser = new tabs.tabBrowser(aomTabs[0].controller);
        tabBrowser.closeTab("middleClick", aomTabs[0].index);
      }
      else {
  ...
  installAddon : function AddonsManager_installAddon(aSpec) {
    ...
    this.scrollAddonIntoView(aSpec);
  ...
  /**
   * Scrolls add-on into View
   *
   * @param  {object} aSpec
   *         Information on which add-on to operate on
   *         Elements: addon - Add-on element
   */
  scrollAddonIntoView : function AddonsManager_scrollAddonIntoView(aSpec){
    var addonsList = this.getElement({ type: 'addonsList' }).getNode();
    addonsList.ensureElementIsVisible(aSpec.addon.getNode());
  },
