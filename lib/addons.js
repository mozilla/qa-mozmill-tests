...
AddonsManager.prototype = {
  ...
  waitForCategory : function AddonsManager_waitForCategory(aSpec) {
    ...
    if (this.selectedCategory.getNode() !== category.getNode())
      throw new Error("Target category has not been set - got '" +
                      this.selectedCategory.getNode().id + "', expected: '" +
                      category.getNode().id + "'");
  },
  ...
  waitForSearchFinished : function AddonsManager_waitForSearchFinished(aSpec) {
    ...
    // Add event listener to wait until the view has been updated
    var self = { updated: false };
    function onViewUpdated() { self.updated = true; }
    this.controller.window.document.addEventListener("ViewChanged",
                                                     onViewUpdated, false);
    ...
    try {
      this.controller.waitFor(function () {
        return self.updated;
      }, "Search has been finished." , timeout, undefined, this);
    } finally {
      this.controller.window.document.removeEventListener("ViewChanged",
                                                          onViewUpdated, false);
    }
