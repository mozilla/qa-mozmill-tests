...
function checkAccessKeysResults(controller, accessKeysSet) {
  ...
  for (var i = 0; i < valueList.length; i++) {
    ...
    if (valueList[i].length > 1) {
      ...
      expect.fail('accessKey: ' + aKeysList[i] + ' found in strings: ' +
                  valueList[i].join(", "));
    }
  }
...
function checkDimensions(child) {
  ...
  if (childBox.height > tolerance &&
      childBox.screenX + childBox.width >
      parentBox.screenX + parentBox.width + tolerance) {
    pixels = childBox.x + childBox.width - parentBox.x - parentBox.width;
    badRects.push([parentBox.x + parentBox.width, childBox.y,
                   pixels, childBox.height]);
    expect.fail('Node is cut off by ' + pixels +
  ...
  if (child.nodeName != 'menupopup' && parent.nodeName != 'menupopup') {
    if (childBox.width > tolerance &&
        childBox.screenY + tolerance < parentBox.screenY) {
      pixels = parentBox.y - childBox.y;
      badRects.push([childBox.x, childBox.y,
                     childBox.width, pixels]);
      expect.fail('Node is cut off by ' + pixels +
                  ' px at the top: ' + _reportNode(child) +
                  '. Parent node: ' + _reportNode(parent));
    }
    if (childBox.width > tolerance &&
        childBox.screenY + childBox.height >
        parentBox.screenY + parentBox.height + tolerance) {
      pixels = childBox.y + childBox.height - parentBox.y - parentBox.height;
