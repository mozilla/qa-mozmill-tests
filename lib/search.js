...
searchBar.prototype = {
  ...
  installEngine: function searchBar_installEngine(aName, aURL, aSpec) {
    ...
    this._bss.addEngineWithDetails(aName, null, null, null,
                                   method, aURL);
    if (selected) {
  ...
  restoreDefaultEngines : function searchBar_restoreDefaults() {
    ...
    var defaults = this._bss.getDefaultEngines({ }).map(function (aEngine) aEngine.name);
    var installedEngines = this._bss.getEngines({ }).map(function (aEngine) aEngine.name);
    var self = this;

    // Remove installed engines that are not default
    installedEngines.forEach(function (aEngine) {
      if (defaults.indexOf(aEngine) === -1)
        self.removeEngine(aEngine);
    });
