...
autoCompleteResults.prototype = {
  ...
  getUnderlinedText : function autoCompleteResults_getUnderlinedText(result, type) {
    ...
    let values = [ ];
    dump("\n Type: " + type + "\n");
    dump("Nodes length: " + description.childNodes.length + "\n");
    for (i = 0; i < description.childNodes.length; i++) {
      dump("\nIteration number: " + i + "\n");
      dump("Node name: " + description.childNodes[i].nodeName + "\n");
      dump("innerHTML: " + description.childNodes[i].innerHTML + "\n");
      if (description.childNodes[i].nodeName == 'span') {
        dump("I got here \n");
        // Only add underlined text to the results
        values.push(description.childNodes[i].innerHTML);
      }
    }
