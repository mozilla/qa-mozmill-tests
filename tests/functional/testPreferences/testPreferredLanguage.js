...
var langHandler = function (aController) {
  ...
  var filtertedLanguageList = languageList.nodes.filter(function (aElement) {
    return initLang.indexOf(aElement.getAttribute("id")) === -1;
  });

  // Pick a random language
  var randomPosition = Math.floor(Math.random() * filtertedLanguageList.length);
  var languageName = filtertedLanguageList[randomPosition].getAttribute("label");
  ...
  assert.waitFor(function () {
    return langDropDown.getNode().label === languageName;
  }, languageName + " has been correctly selected");
